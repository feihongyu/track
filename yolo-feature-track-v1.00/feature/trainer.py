...
class Trainer(object):
    ...
    def __init__(
            ...
            persist_stride=5, lamda=0.03, alpha=0.5, arch="resnet18"):
        ...
        self.lamda = lamda if lamda <= 1 else 0.03
        self.alpha = alpha
        self.arch = arch
    ...
    def run_epoch(self, mode):
        ...
        with torch.set_grad_enabled(mode == 'train'):
            for images, targets, names in dataloader:
                ...
                loss = self.lamda * center_loss + (1 - self.lamda) * cross_entropy_loss
            ...
            if mode == "train":
                train_info = r"plot/train," + str(self.arch) + ",lamda=" + str(self.lamda) + r".txt"
            else:
                train_info = r"plot/validate," + str(self.arch) + ",lamda=" + str(self.lamda) + r".txt"

            with open(train_info, "a+") as f:
                result = str(self.current_epoch) + " " \
                         + str(cross_entropy_loss.item()) + " " \
                         + str(center_loss.item() + loss.item()) + " " \
                         + str(top1_acc * 100) + " " + \
                         str(top3_acc * 100) + "\n"
                f.write(result)
